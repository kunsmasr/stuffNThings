...
namespace StuffNThings.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		[HttpPost]
		public ActionResult GetStatesJSON()
		{
			var locationRepository = new LocationRepository(_connectionString);
			var states = locationRepository.GetStatesByCountryId(Convert.ToInt32(ConfigurationManager.AppSettings["DefaultCountryId"])).Select(sta => new SelectListItem { Text = sta.Name, Value = sta.Id.ToString() });
		...
		private IEnumerable<SelectListItem> GetCountries(){
			var locationRepository = new LocationRepository(_connectionString);
			var countries = locationRepository.GetAllCountries().Select(cou => new SelectListItem { Text = cou.Name, Value = cou.Id.ToString() });
